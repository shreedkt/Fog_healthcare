...
class RecordTamperedError(FogHealthcareException):
    """Raised when a record's integrity check fails on read."""

    default_message = "Record integrity check failed – data may have been tampered with."
...
# ──────────────────────────────────────────────────────────────
# DRF API Exceptions (return proper HTTP responses)
# ──────────────────────────────────────────────────────────────
class IntegrityFailedAPIError(APIException):
    """HTTP 400 – integrity hash mismatch."""

    status_code = status.HTTP_400_BAD_REQUEST
    default_detail = "Integrity verification failed."
    default_code = "integrity_failed"


class ForbiddenAPIError(APIException):
    """HTTP 403 – user lacks required role."""
